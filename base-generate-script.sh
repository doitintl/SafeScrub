...
function usage() {
  ...
  echo "You can direct output to create your deletion script, as for example by  suffixing"
  echo "        > deletion-script.sh  && chmod a+x deletion-script.sh"
  printf "Resources from these APIs are supported: "
  this_script=$(basename "$0")
  grep  "^\s*create_deletion_code" "${this_script}"| cut -d " " -f2 | tr '\n' ', ' | rev | cut -c 2- | rev
  exit 1
}
...
exit 0
compute_resource_types="addresses backend-services firewall-rules forwarding-rules health-checks http-health-checks https-health-checks instance-groups instance-templates instances networks networks routers routes target-pools target-tcp-proxies"
